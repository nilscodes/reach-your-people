...
const ButtonPopoverTrigger = forwardRef(function TriggerRef(props: ButtonPopoverTriggerProps, ref) {
  const { icon, label, variant, fullButton, ...rest } = props;
  if (!fullButton) {
    return (<Tooltip label={label} aria-label={label} hasArrow >
      <IconButton icon={icon} {...rest} aria-label={label} variant={variant} ref={ref} />
    </Tooltip>)
  } else {
    return (<Button leftIcon={icon} {...rest} aria-label={label} variant={variant} ref={ref}>
      {label}
    </Button>)
  }
});

export default function SubscriptionStatusButton({ subscription, fullButton, onStatusChange, ...props }: SubscriptionStatusButtonProps) {
  ...
  return (
    <Popover>
      {({ onClose }) => (
        <>
          <PopoverTrigger>
            <ButtonPopoverTrigger icon={icon} label={label} variant={variant} {...props} fullButton={fullButton} />
          </PopoverTrigger>
          <PopoverContent>
